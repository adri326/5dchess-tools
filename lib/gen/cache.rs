...
/** A wrapper around GenMoves to cache the moves generated by an iterator.
...
/**
    A wrapper around GenMoves to cache the moves and boards generated by an iterator.
    You should consider using this if you wish to re-use a `GenMoves`-iterator
    and if your `Game` and `PartialGame` instances stay the same.

    This structure is notably used by `GenLegalMovesetIter`.
**/
#[derive(Clone)]
...
impl<'a, G: GenMoves<'a>> TryFrom<(G, &'a Game, &'a PartialGame<'a>)> for CacheMoves<G::Iter> {
    ...
    /**
        ...
        // Assign to game and partial_game here

        // This loop iterates on all of the playable boards by the current player and prints their valid moves
        for board in partial_game.own_boards(game) {
            if let Ok(board_iter) = CacheMoves::try_from(board, game, partial_game) {
                for mv in &mut iter {
                    println!("{}", mv);
                }
                println!("The first move was {:?}", iter.get_cached(0));
            }
        }
        ```
    **/
...
impl<I: Iterator<Item = Move>> CacheMoves<I> {
    /**
        Creates a new instance of CacheMoves: the moves yielded by `iter` from then on will be cached.
        If you only wish to also cache the generated boards, you may use `CacheMovesBoards` instead.

        ## Example

        ```
        let timeline: Layer = 0;
        let game: &Game;
        let partial_game: &PartialGame;

        // Assign to game and partial_game here

        // Get the last board from the 0-th timeline
        let time = partial_game.info.get_timeline(timeline).unwrap().last_board;
        let board = partial_game.get_board_with_game(game, (timeline, time));

        let board_iter = board.generate_moves(game, partial_game).unwrap();
        let cache_iter = CacheMoves::new(board_iter);

        println!("First move: {:?}", cache_iter.get(0));
        println!("Second move: {:?}", cache_iter.get(1));
        println!("First move (again): {:?}", cache_iter.get(0)); // This call executes in constant time
        ```
    **/
    ...
    pub fn from_raw_parts(iterator: I, cache: Vec<Move>, done: bool) -> Self {
        Self {
            iterator,
            cache,
            done,
    ...
    **/
    pub fn into_raw_parts(self) -> (I, Vec<Move>, bool) {
...
impl<'a, G: GenMoves<'a>> TryFrom<(G, &'a Game, &'a PartialGame<'a>)> for CacheMovesBoards<'a, G::Iter> {
    ...
    /**
        ...
        // This loop iterates on all of the playable boards by the current player and prints their valid moves
        for board in partial_game.own_boards(game) {
            if let Ok(board_iter) = CacheMovesBoards::try_from(board, game, partial_game) {
                for mv in &mut iter {
                    println!("{}", mv);
                }
                println!("The first move was {:?}", iter.get_cached(0));
                println!("Its generated boards are {:?}", iter.get_board_cached(0));
            }
        }
        ```
    **/
...
impl<'a, I: Iterator<Item = Move>> CacheMovesBoards<'a, I> {
    /**
        Creates a new instance of CacheMovesBoards: the moves yielded by `iter` from then on will be cached, alongside with their generated boards.
        If you only wish to cache the moves themselves, then you may use `CacheMoves` instead.
        ...
        let board_iter = board.generate_moves(game, partial_game).unwrap();
        let cache_iter = CacheMovesBoards::new(board_iter, game, partial_game);

        println!("Move: {:?}", cache_iter.get(0));
        println!("Generated boards: {:?}", cache_iter.get_board(0));
        ```
    **/
    ...
    /**
        Allows the creation of a new `CacheMovesBoards` from a tuple with its fields.
        You most likely want to use `CacheMovesBoards::new(...)` instead.
    **/
    ...
    **/
    ...
    /**
        Returns the n-th board pair (corresponding to the boards generated by the n-th move) from the cache, or None if the index is outside of the cache.
    **/
    pub fn get_board_cached(&self, n: usize) -> Option<(Cow<'_, Board>, Cow<'_, Option<Board>>)> {
        if n < self.cache.len() {
    ...
    /**
        Returns the n-th board pair; it first looks if it is already in the cache.
        Otherwise, the iterator is consumed up to the n-th move and its corresponding, generated board pair is returned.
    **/
    ...
    **/
    ...
    /**
        Returns the underlying raw parts. Only use this if you know what you're doing!
    **/
    pub fn into_raw_parts(self) -> (I, Vec<Move>, Vec<(Board, Option<Board>)>, bool, &'a Game, &'a PartialGame<'a>) {
