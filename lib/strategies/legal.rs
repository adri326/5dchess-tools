    ...
    for<'b> &'b B: GenMoves<'b, B>,
{
    ...
    fn apply(&self, mv: Move, game: &'a Game, partial_game: &'a PartialGame<'a, B>) -> Option<bool> {
        ...
        if mv.from.1.non_physical() == mv.to.1.non_physical() {
            ...
            mv.generate_partial_game(game, partial_game, &mut new_partial_game, PartialGameGenKind::Target);
            ...
            mv.generate_partial_game(game, partial_game, &mut new_partial_game, PartialGameGenKind::Source);
...
fn filter_non_physical_move<'a, B>(game: &'a Game, partial_game: &'a PartialGame<'a, B>) -> Option<bool>
where
    B: Clone + AsRef<Board>,
    for<'b> B: From<(Board, &'b Game, &'b PartialGame<'b, B>)>,
    for<'b> &'b B: GenMoves<'b, B>,
