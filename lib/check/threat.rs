...
**/
pub fn is_threatened<'a>(
    game: &'a Game,
    partial_game: &'a PartialGame<'a>,
) -> Option<(bool, Option<Move>)> {
...
#[macro_use]
mod macros {
    /// Macro used to easily run and handle the function `cast_rider_treats` with similar inputs
    ...
    /// Internal macro, expanded by `cast_rider_threats!`
    macro_rules! cast_rider_threats_sub {
        ( $game:expr, $partial_game:expr, $board:expr, $dl:expr, $dt:expr, [$(($index:expr, $dx:expr, $dy:expr)),* $(,)?] ) => {
            let mut tmp_bitboards = if $board.white() {
                [$(
                    $board.bitboards.white[$index] as BitBoardPrimitive
                ),*]
            } else {
                [$(
                    $board.bitboards.black[$index] as BitBoardPrimitive
                ),*]
            };
            let tmp_dx = if $board.white() {
                [$(
                    $dx as Physical
                ),*]
            } else {
            ...
            if let Some((from_x, from_y, to)) = cast_rider_threats($game, $partial_game, $board.l(), $board.t(), $dl, $dt * 2, &mut tmp_bitboards, &tmp_dx, &tmp_dy) {
                return Some((
                    true,
    ...
    /// Macro used to easily run and handle `cast_leaper_threats` with similar inputs
    ...
    /// Internal macro expanded by `cast_leaper_threats!`
    macro_rules! cast_leaper_threats_sub {
        ( $game:expr, $partial_game:expr, $board:expr, $dl:expr, $dt:expr, [$(($index:expr, $dx:expr, $dy:expr)),* $(,)?] ) => {
            ...
            if let Some((from_x, from_y, to)) = cast_leaper_threats($game, $partial_game, $board.l() + $dl, $board.t() + $dt * 2, &mut tmp_bitboards, &tmp_dx, &tmp_dy) {
                return Some((
                    true,
                    Move::new($game, $partial_game, Coords($board.l(), $board.t(), from_x, from_y), to),
                ));
    ...
    /// Macro to easily shift a leaper bitboard and handle its intersections
...
pub fn is_threatened_bitboard<'a>(
    game: &'a Game,
    partial_game: &'a PartialGame<'a>,
) -> Option<(bool, Option<Move>)> {
    for board in partial_game.opponent_boards(game) {
        if let Some(res) = is_threatened_bitboard_board(game, partial_game, board) {
            return Some(res);
        }
    }
...
/// Looks at all of the threatening moves originating from the given board, using bitboards.
pub fn is_threatened_bitboard_board<'a>(
    game: &'a Game,
    partial_game: &'a PartialGame<'a>,
    board: &'a Board,
) -> Option<(bool, Option<Move>)> {
    if let Some((from_x, from_y, to_x, to_y)) = threats_within_board(board) {
        return Some((
            ...
            Move::new(
                game,
                partial_game,
                Coords(board.l(), board.t(), from_x, from_y),
                Coords(board.l(), board.t(), to_x, to_y),
            ),
        ));
    }
...
/// Looks at the threatening moves from rider pieces, given a ⟨dl, dt⟩ non-physical vector and a set of bitboards to manipulate
#[inline]
fn cast_rider_threats<'a>(
    ...
    dl: Layer,
    dt: Time,
    bitboards: &mut [BitBoardPrimitive],
    dx: &[Physical],
    dy: &[Physical],
    ...
    loop {
        ...
        match partial_game.get_board_with_game(game, (l, t)) {
            Some(board) => {
                ...
                let mut zero = true;
                for o in 0..bitboards.len() {
                    let bitboard = &mut bitboards[o];
                    let dx = dx[o];
                    let dy = dy[o];
                    *bitboard =
                        bitboard_shift(*bitboard, dx, dy, board.width(), board.height()) & movable;
                    let attack = *bitboard & royal;
                    if attack != 0 {
...
/// Looks at the threatening moves from leaper pieces, given a ⟨dl, dt⟩ non-physical vector and a set of bitboards to manipulate
#[inline]
fn cast_leaper_threats<'a>(
    ...
    match partial_game.get_board_with_game(game, (l, t)) {
        Some(board) => {
            ...
            for n in 0..bitboards.len() {
                let bitboard = &mut bitboards[n];
                let dx = dx[n];
                let dy = dy[n];
                *bitboard =
                    bitboard_shift(*bitboard, dx, dy, board.width(), board.height()) & movable;
