...
impl Board {
    ...
    **/
    pub fn new(
        ...
        let fits_bitboards = (width as u32) * (height as u32) <= 128;
...
pub struct BitBoards {
    // White's pieces
    pub white: [u128; N_BITBOARDS],
    pub white_royal: u128,
    pub white_movable: u128,

    // Black's pieces
    pub black: [u128; N_BITBOARDS],
    pub black_royal: u128,
    pub black_movable: u128,
}

impl BitBoards {
    pub fn set(&mut self, mask: &BitBoardMask, shift: u32) {
        for n in 0..N_BITBOARDS {
            self.white[n] = (self.white[n] & !(1u128 << shift)) | (mask.white[n] as u128) << shift;
            self.black[n] = (self.black[n] & !(1u128 << shift)) | (mask.black[n] as u128) << shift;
        }

        self.white_royal = (self.white_royal & !(1u128 << shift)) | (mask.white_royal as u128) << shift;
        self.black_royal = (self.black_royal & !(1u128 << shift)) | (mask.black_royal as u128) << shift;

        self.white_movable = (self.white_movable & !(1u128 << shift)) | (mask.white_movable as u128) << shift;
        self.black_movable = (self.black_movable & !(1u128 << shift)) | (mask.black_movable as u128) << shift;
    }

    /// Assumes that `pieces` fits!
    pub fn from_pieces(pieces: &Vec<Tile>) -> Self {
        let mut white = [0u128; N_BITBOARDS];
        let mut white_royal = 0u128;
        let mut white_movable = 0u128;

        let mut black = [0u128; N_BITBOARDS];
        let mut black_royal = 0u128;
        let mut black_movable = 0u128;

        for n in 0..(pieces.len() as u32) {
            let mask = pieces[n as usize].bitboard_mask();
            for o in 0..N_BITBOARDS {
                white[o] |= (mask.white[o] as u128) << n;
                black[o] |= (mask.black[o] as u128) << n;
            }
            white_royal |= (mask.white_royal as u128) << n;
            white_movable |= (mask.white_movable as u128) << n;
            black_royal |= (mask.black_royal as u128) << n;
            black_movable |= (mask.black_movable as u128) << n;
        }
