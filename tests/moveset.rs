...
fn test_moveset() {
    ...
    let moves: HashSet<Move> = game.get_board((0, 1)).unwrap().generate_moves(&game, &partial_game).unwrap().collect();

    let mut yielded = false;

    for ms in iter.flatten().filter_map(|ms| ms.ok()) {
        yielded = true;
        assert!(ms.moves().len() == 1, "Expected moveset {:?} to only contain one move", ms);
    ...
    assert!(yielded, "Expected moveset generator to return at least one moveset!");
}
