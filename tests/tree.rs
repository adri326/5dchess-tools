...
lazy_static! {
    pub static ref OPTIONS: TasksOptions<ContinueGoal> = TasksOptions::default().n_threads(N_THREADS).max_duration(Some(Duration::new(20, 0)));
}
...
fn test_dfs_rook_tactics_1() {
    ...
    let res = dfs(&game, node, 1, Some(Duration::new(10, 0)), NoEvalFn::new(), ContinueGoal, false);
    assert!(res.is_some(), "dfs timed out or errored out on rook-tactics-1!");
    assert_eq!(res, dfs_schedule(&game, 1, NoEvalFn::new(), *OPTIONS), "dfs_schedule to return the same value as dfs");
...
fn test_dfs_rook_tactics_2() {
    ...
    let res = dfs(&game, node.clone(), 3, Some(Duration::new(30, 0)), NoEvalFn::new(), ContinueGoal, false);
    assert!(res.is_some(), "dfs timed out or errored out on rook-tactics-2!");
    let dfs_res = dfs_schedule(&game, 3, NoEvalFn::new(), *OPTIONS).expect("dfs_schedule should return Some(...)");
    assert!(dfs_res.0.path[0] == solution_1 || dfs_res.0.path[0] == solution_2, "dfs_schedule should return a valid solution");
    let iddfs_res = iddfs(&game, node, Some(Duration::new(30, 0)), NoEvalFn::new(), ContinueGoal, false).expect("iddfs should return Some(...)");
...
fn test_dfs_standard_1() {
    ...
    let res = dfs(&game, node, 3, Some(Duration::new(20, 0)), NoEvalFn::new(), ContinueGoal, false);
...
fn test_dfs_rook_tactics_3() {
    ...
    let res = dfs_bl(&game, node, 3, 1, Some(Duration::new(30, 0)), NoEvalFn::new(), ContinueGoal, false);
...
fn test_dfs_advanced_branching_2() {
    ...
    let res = dfs(&game, node, 1, Some(Duration::new(20, 0)), NoEvalFn::new(), ContinueGoal, false);
